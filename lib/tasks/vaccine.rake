      ...
      results[:centers].each do |center|
        ...
        if sessions_with_availability.any?
          ...
          sessions_with_availability.each do |s|
            message =  "Slot Available at #{center[:name]}, #{center[:address]}, #{center[:state_name]}, #{center[:block_name]}, Pincode - #{center[:pincode]}, Total Slot Available - #{s[:available_capacity_dose1]}"
